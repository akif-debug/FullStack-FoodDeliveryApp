...
const RowContainer = ({ flag, data, scrollValue }) => {
    ...
    return (
        <div ref={rowContainer} className={`w-full flex items-center gap-3 my-12  scroll-smooth ${flag ? 'overflow-x-scroll scrollbar-none' : 'overflow-x-hidden flex-wrap justify-center'}`}>
            {data && data.length > 0 ? (
                data.map((item) => (
                    <div key={item?.id} className="w-300 h-[225px] min-w-[300px] md:w-340 md:min-w-[340px] md:h-auto rounded-lg p-2 my-12 backdrop-blur-lg bg-cardOverlay hover:drop-shadow-lg flex flex-col items-center justify-between">
                        <div className="w-full flex items-center justify-between">
                            <motion.div whileHover={{ scale: 1.2 }} className='w-40 h-40 -mt-8 drop-shadow-2xl'>
                                <img className='w-full h-full object-contain' src={item?.imageURL} />
                            </motion.div>
